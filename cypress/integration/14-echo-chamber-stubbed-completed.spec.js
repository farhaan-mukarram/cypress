...
describe('Signing in with a seeded database', () => {
  ...
  it('should navigate to the URL of the post that you clicked on', () => {
    cy.get('@previews')
      ...
      .parent()
      .then(($parentElem) => {
        const link = $parentElem.attr('href');
  ...
  it('should send a POST request when submitting the form', () => {
    cy.get('@newPostInput').type('Testing hot takes{enter}');

    cy.wait('@createPostApi').its('request.method').should('eq', 'POST');
  });

  describe('An individual post', () => {
    beforeEach(() => {
      cy.intercept('PATCH', '/echo-chamber/api/1', {
        statusCode: 201,
      }).as('updatePostApi');

      cy.intercept('DELETE', '/echo-chamber/api/1', {
        statusCode: 201,
      }).as('deletePostApi');

      cy.visit('/echo-chamber/posts/1');

      cy.get('[data-test="post-detail"]').as('individualPostPreview');

      cy.get('[data-test="post-detail-controls-edit-button"]').as('postEditButton');

      cy.get('[data-test="post-detail-controls-delete-button"]').as('postDeleteButton');
    });

    it('should show an edit field when you click on the edit button', () => {
      cy.get('@postEditButton').click();
    ...
    it('should send a PATCH request when you send your edit', () => {
      ...
      cy.get('[data-test="post-detail-draft-content"]').type('post test edit{enter}');

      cy.wait('@updatePostApi').its('request.method').should('eq', 'PATCH');
    });

    it('should send a DELETE request when click on the delete button', () => {
      cy.get('@postDeleteButton').click();

      cy.wait('@deletePostApi').its('request.method').should('eq', 'DELETE');
    });
  });
});
